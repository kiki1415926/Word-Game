...
char **prune_word_list(char **words, int len, int *words_remaining) {
    ...
    *words_remaining = length;
    char **word_list = malloc(((*words_remaining) + 1) * sizeof(char*));
    if (word_list == NULL) {
        perror("malloc");
        exit(1);
    }
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    *len = length;
    int count;
    char **word_list = prune_word_list(words, length, &count);
    while(count == 0){
        printf("There are no words of that length.\n");
        printf("Length of words to use? ");
        ...
        word_list = prune_word_list(words, length, &count);
    }
    return word_list;
}
